...
def resume_env(plot=False,  # To plot results (Field, controls, lift, drag, rec area) during training
               dump_vtu=20,  # If not False, create vtu files of area, velocity, pressure, every 'dump_vtu' steps
               dump_debug=1,  # If not False, output step info of ep,step,rec_area,L,D,jets Q* to saved_models/debug.csv, every 'dump_debug' steps
    ...
    def _init():
        ...
        optimization_params = {"num_steps_in_pressure_history": 1,
                               ...
                               "min_value_jet_MFR": -0.15,  # Set min and max Q* for weak actuation
                               "max_value_jet_MFR": 0.15,
        ...
        env_2d_cylinder = Monitor(TimeLimit(Env2DCylinderModified(path_root=root,
                                                ...
                                                simu_name=simu_name), max_episode_steps = 1000000000))
